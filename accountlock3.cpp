#include "accountlock3.hpp"


void accountlock3::lock(name target_contract, const time_point_sec &unlock_time, uint8_t days_notice, string public_key_str, string unlocker_str, string locked_auth) {
    //check for simultaneous actions
    //avoids exploit that crafts a tx with a lock and then an updateauth action, which succeeds and adds a lock entry to the table, but the account isn't actually really locked
    ...
    check(trx.actions.size() == 1, "No simultaneous actions permitted");


    require_auth(target_contract);
    if(unlocker_str != "") check(name(unlocker_str) != "accountlock3"_n, "please specify a valid unlocker name"); //this also validates name(unlocker_str)
    ...
    auto permlev = permission_level{"accountlock3"_n , "eosio.code"_n };
...
void accountlock3::unlock(name target_contract) {
...
void accountlock3::ontransfer(name from, name to, asset quantity, string memo) {
